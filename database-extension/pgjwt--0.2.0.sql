...
CREATE OR REPLACE FUNCTION algorithm_sign(signables text, secret text, algorithm text)
  ...
  WITH alg AS(
    ...
    SELECT
      public.url_encode(public.hmac(signables, secret, alg.id))
    FROM
      alg;
$$ IMMUTABLE;
...
CREATE OR REPLACE FUNCTION sign (payload json, secret text, algorithm text DEFAULT 'HS256')
  ...
  WITH header AS(
    SELECT
      public.url_encode(convert_to('{"alg":"' || algorithm || '","typ":"JWT"}', 'utf8')) AS data
),
payload AS(
  SELECT
    public.url_encode(convert_to(payload::text, 'utf8')) AS data
),
signables AS(
  SELECT
...
SELECT
  signables.data || '.' || public.algorithm_sign(signables.data, secret, algorithm)
FROM
  signables;
$$ IMMUTABLE;
...
CREATE OR REPLACE FUNCTION verify(token text, secret text, algorithm text DEFAULT 'HS256')
  ...
  SELECT
    ...
    jwt.signature_ok
    AND tstzrange(to_timestamp(public.try_cast_double(jwt.payload ->> 'nbf')), to_timestamp(public.try_cast_double(jwt.payload ->> 'exp'))) @> CURRENT_TIMESTAMP AS valid
  FROM(
    SELECT
